...
def _set_user_filtro(current_filtros: Optional[str], rut: str) -> str:
    """Set or replace the user= param in a filtros query string, preserving other params."""
    params = parse_qs(current_filtros or "", keep_blank_values=True)
    params["user"] = [rut]
    ...
    def process_users(self, db: Session, users: List[dict]):
        ...
        for item in users:
            ...
            if existing:
                ...
                existing.filtros = _set_user_filtro(existing.filtros, username)
                updated += 1
                ...
                new_user = models.User(
                    ...
                    role="user",
                    is_admin=False,
                    filtros=_set_user_filtro(None, username),
                )
                db.add(new_user)
                created += 1
